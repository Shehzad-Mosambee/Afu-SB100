...
typedef enum 
{
    FILESYS_SPI_COMMOM = 0,   //通用SPI
    FILESYS_SPI_EXT,		      //内部flash扩展SPI
    FILESYS_SPI_MAX,
}FILE_SPI_TYPE_E;

typedef enum 
{
	FILESYS_SPI_EXT_BOAD_DEFAULT,
	FILESYS_SPI_EXT_BOAD_83M,
	FILESYS_SPI_EXT_BOAD_71M,
	FILESYS_SPI_EXT_BOAD_62M,
	FILESYS_SPI_EXT_BOAD_41M,
	FILESYS_SPI_EXT_BOAD_MAX,
}FILE_SPI_EXT_BUAD_E;
...
typedef struct 
{
	uint32 buad;
	uint32 dma_flag;
}FILE_SPI_COMMOM_CONFIG_T;


typedef struct
{
	FILE_SPI_TYPE_E type;
	FILE_SPI_COMMOM_CONFIG_T commom_config;
	FILE_SPI_EXT_BUAD_E	buad;
}FILE_SPI_CONFIG_T;
...
typedef struct halSpiFlash
{
    ...
    uint8 sreg_min_num : 4;
    /** maximum security register number */
    uint8 sreg_max_num : 4;
...
/*********************************************************************************************
 * 用户自定义文件系统盘符   只能"E:"、"D:"、"F:"    这类，区分大小写。
 * 操作文件的时候传完整路径
 * *******************************************************************************************/

INT InitFileSystemPlusAndPoint(LPCTSTR lpPoint, uint32 startAddr,FILESYS_CAPACITY_E fileSysCap,
							FILESYS_CAPACITY_E spiFlashCap);

INT FomatDisk_Point(LPCTSTR point);
INT GetDeviceFreeSpace_Point(uint32 * pDiskFreeSize, LPCTSTR point);
INT GetDeviceUsedSpace_Point(uint32 * pDiskUsedSize, LPCTSTR point);
HANDLE FindFirstFile_Point(LPCTSTR lpFileName, LPMCFILE_FIND_DATA_T lpFindFileData, LPCTSTR point);


/*********************************************************************************************
 * 设置文件系统使用通用SPI 或者扩展SPI （系统默认使用通用spi）
 * *******************************************************************************************/
int FileSpi_Config(FILE_SPI_CONFIG_T *pConfig);

/*********************************************************************************************
 * 使用 FILESYS_SPI_EXT 扩展 Flash 用到该函数
 * *******************************************************************************************/
int SetFlashChipData(halSpiFlash_t *pConfig);

int Format_AllPoint(void);

/*********************************************************************************************
 * 获取文件系统 使用的spi 相关参数
 * *******************************************************************************************/
int GetFileSpi_Config(FILE_SPI_CONFIG_T *pConfig);
#ifdef __cplusplus
}
#endif /* __cplusplus */
